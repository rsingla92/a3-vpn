...
inverse_bytesub = [['0x52', '0x09', '0x6a', '0xd5', '0x30', '0x36', '0xa5', '0x38', '0xbf', '0x40', '0xa3', '0x9e', '0x81', '0xf3', '0xd7', '0xfb'],
...
mixcolumn_table2 = [0x00,0x02,0x04,0x06,0x08,0x0a,0x0c,0x0e,0x10,0x12,0x14,0x16,0x18,0x1a,0x1c,0x1e,
...
mixcolumn_table3 = [0x00,0x03,0x06,0x05,0x0c,0x0f,0x0a,0x09,0x18,0x1b,0x1e,0x1d,0x14,0x17,0x12,0x11,
...
mixcolumn_table9 = [0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77,
...
mixcolumn_table11 = [0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69,
...
mixcolumn_table13 = [0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b,
...
mixcolumn_table14 = [0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a,
...
def bytesub(hex_val, inv=False):
    ...
    if isinstance(hex_str, int):
        hex_str = hex(hex_val)
    number_pair = hex_str.split('x')[1]
    if len(number_pair) == 1:
        number_pair = "0" + number_pair
    table = bytesub_table
    if inv:
        table = inverse_bytesub
    return table[int(number_pair[0], 16)][int(number_pair[1], 16)]

def copy_list_of_lists(list_of_lists):
    new_list = []
...
def bytesub_transform(data_block, inv=False):
    ...
    for row, dat in enumerate(data_block):
        for col, byte in enumerate(dat):
            if isinstance(byte, str):
                byte = int(byte, 16)
            transformed[row][col] = int(bytesub(byte, inv), 16)
    return transformed
...
def shift_row(data_block, inv=False):
    ...
    for i in range(4):
        ...
        if not inv:
            shifter = -i
        transformed[i] = rot_list(data_block[i], shifter)
    return transformed
...
def mix_column(column, inv=False):
    ...
    if not inv:
        ...
        b1 = column[0] ^ mixcolumn_table2[column[1]] ^ mixcolumn_table3[column[2]] ^ column[3]
        b2 = column[0] ^ column[1] ^ mixcolumn_table2[column[2]] ^ mixcolumn_table3[column[3]]
        b3 = mixcolumn_table3[column[0]] ^ column[1] ^ column[2] ^ mixcolumn_table2[column[3]]
    else:
        b0 = mixcolumn_table14[column[0]] ^ mixcolumn_table11[column[1]] ^ mixcolumn_table13[column[2]] ^ mixcolumn_table9[column[3]]
        b1 = mixcolumn_table9[column[0]] ^ mixcolumn_table14[column[1]] ^ mixcolumn_table11[column[2]] ^ mixcolumn_table13[column[3]]
        b2 = mixcolumn_table13[column[0]] ^ mixcolumn_table9[column[1]] ^ mixcolumn_table14[column[2]] ^ mixcolumn_table11[column[3]]
        b3 = mixcolumn_table11[column[0]] ^ mixcolumn_table13[column[1]] ^ mixcolumn_table9[column[2]] ^ mixcolumn_table14[column[3]]
    return [b0, b1, b2, b3]
